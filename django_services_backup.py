...
from django.utils import timezone
from apps.analytics.models import AnalyticsEvent, UserSessionAnalytics
from apps.templates.models import PerformanceMetrics
import uuid
...
class AnalyticsService:
    ...
    def track_event(user=None, event_name=None, category='general', properties=None, 
        ...
        AnalyticsEvent.objects.create(
            user=user,
            event_type=category or 'page_view',  # Use event_type instead of category
            event_name=event_name,
            properties=properties or {},
            session_id=session_id or str(uuid.uuid4()),
            metadata=device_info or {}  # Use metadata instead of device_info
        )
    ...
    def start_session(user=None, device_info=None):
        ...
        UserSessionAnalytics.objects.create(
    ...
    def end_session(session_id):
        """End a user session"""
        try:
            session = UserSessionAnalytics.objects.get(session_id=session_id)
            session.end_time = timezone.now()
            session.duration_seconds = (session.end_time - session.start_time).total_seconds()
            session.save()
        except UserSessionAnalytics.DoesNotExist:
            pass
