...
web: python manage.py migrate --settings=promptcraft.settings.heroku --noinput && python manage.py collectstatic --settings=promptcraft.settings.heroku --noinput && gunicorn promptcraft.wsgi --bind 0.0.0.0:$PORT --workers 2 --timeout 120 --log-file -
